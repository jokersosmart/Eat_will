      ...
      <main className="relative z-10 container pb-16 pt-7 lg:pt-12">
        ...
        {view === "plan" && <section className="grid gap-6 xl:grid-cols-[.92fr_1.08fr]">
          ...
          <div className="space-y-5">
            ...
            {adviceMutation.data && <section className="rounded-[1.5rem] border border-[#ddcca1] bg-[#fffaf0]/95 p-5 shadow-sm sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold tracking-[.15em] text-[#ad8330]">KNOWLEDGE-BOUND ADVICE</p><h3 className="mt-1 font-serif text-2xl font-bold">智慧建議</h3></div><ShieldCheck className="text-[#ad8330]" /></div><p className="mt-4 border-l-2 border-[#caa75b] pl-4 text-sm leading-7 text-[#334e68]">{adviceMutation.data.executiveSummary}</p><div className="mt-5 grid gap-3">{adviceMutation.data.suggestedTopics.map((topic, index) => <article key={`${topic.title}-${index}`} className="rounded-xl bg-[#f7f1e3] p-4"><p className="font-bold text-[#102a43]">{topic.title}</p><p className="mt-1 text-sm font-medium leading-6 text-[#7a5a1d]">「{topic.question}」</p><p className="mt-1 text-xs leading-5 text-[#667085]">依據：{topic.reasoning}</p></article>)}</div><div className="mt-5 grid gap-3 lg:grid-cols-2"><article className="rounded-xl border border-[#e3d6b6] bg-[#fffdf6] p-4"><p className="text-[10px] font-bold tracking-[.14em] text-[#ad8330]">角色應對</p><div className="mt-2 space-y-2">{adviceMutation.data.roleStrategy.map((item) => <p key={item.role} className="text-xs leading-5 text-[#52657a]"><strong className="text-[#102a43]">{item.role}</strong>：{item.action}</p>)}</div></article><article className="rounded-xl border border-[#e3d6b6] bg-[#fffdf6] p-4"><p className="text-[10px] font-bold tracking-[.14em] text-[#ad8330]">風險提醒</p><ul className="mt-2 space-y-2">{adviceMutation.data.riskWatch.map((item, index) => <li key={index} className="text-xs leading-5 text-[#52657a]">• {item}</li>)}</ul></article></div><article className="mt-5 rounded-xl border border-[#caa75b] bg-[#f4e8cd]/70 p-4"><p className="text-[10px] font-bold tracking-[.14em] text-[#7a5a1d]">局後復盤摘要</p><p className="mt-2 text-sm leading-6 text-[#334e68]">{adviceMutation.data.postDinnerSummary}</p><p className="mt-4 text-[10px] font-bold tracking-[.14em] text-[#7a5a1d]">復盤問題</p><ul className="mt-2 space-y-1.5">{adviceMutation.data.reflectionPrompts.map((item, index) => <li key={index} className="text-xs leading-5 text-[#52657a]">{index + 1}. {item}</li>)}</ul></article><p className="mt-4 text-xs leading-5 text-[#667085]">依據條目：{adviceMutation.data.sourceTitles.join("、")}</p></section>}
        ...
        {view === "library" && <section><div className="rounded-[1.5rem] border border-[#ddcca1] bg-[#fffaf0]/90 p-5 sm:p-7"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-[10px] font-bold tracking-[.15em] text-[#ad8330]">SIX-PART KNOWLEDGE LIBRARY</p><h3 className="mt-1 font-serif text-3xl font-bold">把底層邏輯留在桌上</h3><p className="mt-2 text-sm leading-6 text-[#52657a]">六大部分、課程原始類別與完整策略內容均可在這裡查閱。</p></div><span className="rounded-full bg-[#eadfc5] px-4 py-2 text-xs font-bold text-[#7a5a1d]">{libraryMeta.data?.total ?? 32} 筆策略條目</span></div><div className="mt-6 flex gap-2 overflow-x-auto pb-2">{["全部", ...sections].map((item) => <button key={item} onClick={() => setLibrarySection(item)} className={`shrink-0 rounded-full px-3.5 py-2 text-xs font-bold transition ${librarySection === item ? "bg-[#102a43] text-white" : "border border-[#d8cfae] bg-[#fffdf6] text-[#52657a] hover:bg-[#f4e8cd]"}`}>{item}</button>)}</div><div className="mt-2 flex gap-2 overflow-x-auto pb-2">{["全部", ...(libraryMeta.data?.categories ?? [])].map((item) => <button key={item} onClick={() => setLibraryCategory(item)} className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-medium transition ${libraryCategory === item ? "bg-[#f4e8cd] text-[#7a5a1d]" : "text-[#667085] hover:text-[#102a43]"}`}>{item}</button>)}</div></div><div className="mt-5 grid gap-4 md:grid-cols-2">{libraryEntries.isLoading ? <Loader2 className="animate-spin text-[#ad8330]" /> : (libraryEntries.data ?? []).map((entry) => <article key={entry.id} className="rounded-[1.25rem] border border-[#ded3ba] bg-[#fffdf6]/90 p-5"><div className="flex flex-wrap gap-2"><span className="rounded-full bg-[#eadfc5] px-2.5 py-1 text-[10px] font-bold tracking-[.1em] text-[#7a5a1d]">{entry.section}</span><span className="rounded-full border border-[#e2d8c1] px-2.5 py-1 text-[10px] font-bold text-[#667085]">{entry.category}</span></div><h4 className="mt-3 font-serif text-xl font-bold text-[#102a43]">{entry.title}</h4><p className="mt-3 text-sm leading-7 text-[#52657a]">{entry.content}</p><div className="mt-4 flex flex-wrap gap-1.5 text-[10px] font-semibold text-[#8c6821]"><span className="rounded bg-[#f7f1e3] px-2 py-1">{entry.peopleTags}</span><span className="rounded bg-[#f7f1e3] px-2 py-1">{entry.cuisineTags}</span><span className="rounded bg-[#f7f1e3] px-2 py-1">{entry.timeTags}</span></div></article>)}{!libraryEntries.isLoading && (libraryEntries.data ?? []).length === 0 && <EmptyState title="目前沒有相符條目" description="請改選另一個六大部分或類別查看。" />}</div></section>}
